...
-- It turns out that the result had 25 different rows. Most of them had the pickup time equal to the dropoff time, 
-- so it might be that the taxi driver started these runs by mistake. For the other cases, 
-- I went ahead and picked one sample to check if it was really a duplicate entry.
SELECT *
FROM trips
...
-- Therefore, we can conclude that there are indeed inconsistencies in the data and one interesting thing about 
-- it is that it seems like that people can be charged multiple times for a ride due to umpredictable technical failures.
